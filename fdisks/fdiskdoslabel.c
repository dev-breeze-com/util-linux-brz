				 ...
				 sector_t first[], sector_t last[])
{
	...
	/* Could add code here to get the start sector for batch commands.
	 * Instead of flowing through read_int in fdisks/fdisk.c
	 */
...
static int add_partition(struct fdisk_context *cxt, int n, struct fdisk_parttype *t)
{
	...
	do {
		...
		if (!read && start == temp) {
			...
			start = read_int(cxt, cround(cxt, j), cround(cxt, dflt), cround(cxt, limit), 0, mesg);
			...
			struct fdisk_parttype *t)
{
	...
	if ( cxt->batch.enabled ) {
		struct fdisk_batch_record *rec = cxt->batch.lines[ cxt->batch.curline ];

		if ( rec->logical )
			rc = add_logical( cxt );
		else {
			rc = add_partition( cxt, cxt->batch.curline, &( rec->pt ));
		}
