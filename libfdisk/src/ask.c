...
int fdisk_get_batch_partnum(struct fdisk_context *ctx, size_t *partnum)
{
	 struct fdisk_batch_record *rec = ctx->batch.lines[ ctx->batch.curline ];

	if ( rec ) {
		//fprint( stdout, "partno: %d\n", rec->partno);
		(*partnum) = rec->partno;
		return 0;
	}
	return (-1);
...
int fdisk_ask_partnum(struct fdisk_context *cxt, size_t *partnum, int wantnew)
{
	...
	size_t i, len = sizeof(range);
	size_t begin = 0, run = 0;
	...
	if ( cxt->batch.enabled )
		return fdisk_get_batch_partnum( cxt, partnum );
